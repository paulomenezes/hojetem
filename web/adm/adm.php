<?php
	...
	$sqlVisited = "SELECT s.*, t.name as NomeCategoria FROM store AS s INNER JOIN store_type as t ON s.idStoreType = t.id ORDER BY s.idStoreType ASC, s.event_date DESC";
		...
		<div class="container-fluid" style="margin: 0 auto; width: 1000px;">
			<div class="row">
				<div class="col-sm-12 main">
					...
					<?php $ctg = ""; $old = false; for($i = 0; $i < sizeof($lojas); $i++) { ?>
						<?php 
						...
						<?php 
							$today = date("Y-m-d");
							if ($lojas[$i]['event_date'] < $today && $old == false) {
							...
							<div style="float: left; width: 370px;">
								<div class="media-body">
									<h4 class="media-heading">
										<a href="ver.php?id=<?php echo $lojas[$i]['id']; ?>" style="color:black;font-size:18px;"><?php echo $lojas[$i]['name']; ?></a>
									</h4>
									<?php 
										$today = date("Y-m-d");
										if ($lojas[$i]['event_date'] < $today) {
											echo date('d/m/Y', strtotime($lojas[$i]['event_date'])) . ' - EVENTO ANTIGO'; 
										} else {
											echo date('d/m/Y', strtotime($lojas[$i]['event_date'])); 
										}
									?>
								</div>
							</div>
