<?php
	...
	{   
		...
		$sql_ads = "SELECT * FROM ads WHERE position = 'carrousel'";
		...
		if($_GET) {
			if ($_GET['ac'] == "remover") {
				if ($_GET['di']) {
				    ...
				    header("Location:ads-carrousel.php?msg=remover");
				}
			}
		}
		...
		if($_POST) 
		{
			if($_FILES['image'])
			{
				...
				{   
					...
					{ 
						header("Location:ads-carrousel.php?msg=imagem");
					}  
					else
					{
						...
						if ($image_width == "960" && $image_height == "350") {
							preg_match("/\.(gif|bmp|png|jpg|jpeg){1}$/i", $foto["name"], $ext);   
						
							$nome_imagem = md5(uniqid(time())) . "." . $ext[1];   
							$caminho_imagem = "../images/top/carrousel/" . $nome_imagem;   
						
							move_uploaded_file($foto["tmp_name"], $caminho_imagem);   

							$sql_insert = "INSERT INTO ads (idstore, type, file, link, position) VALUEs (?, ?, ?, ?, 'carrousel')";
						    ...
						    header("Location:ads-carrousel.php?msg=sucesso");
						} else {
							header("Location:ads-carrousel.php?msg=tamanho");
						}
					}
				} else {
					header("Location:ads-carrousel.php?msg=name");					
				}
			} else {
				header("Location:ads-carrousel.php?msg=image");					
			}
		}
	}
	...
	<body>
		...
		<div class="container-fluid" style="margin: 0 auto; width: 1000px;">
			<div class="row">
				...
				<h1 class="page-header">Enviar anúncio carrousel</h1>
				<div class="alert alert-info">Tamanho da imagem: 960 x 350 pixels</div>
				...
				<div id="menu">
					...
					<?php for($i = 0; $i < sizeof($ads); $i++) { ?>
						<div class="well well-sm" style="margin-bottom: 10px;">
							<a href="">
								<img class="media-object" src="../images/top/carrousel/<?php echo $ads[$i]['file']; ?>" style="width:960px;height:350px;margin-right:10px">
							</a>
							<div>
								<div class="media-body">
									...
									<a href="ads-carrousel.php?ac=remover&di=<?php echo $ads[$i]['id']; ?>" onclick="return confirm('Tem certeza?')" class="btn btn-danger">Remover</a>
								</div>
							</div>
							<div class="clear"></div>
						</div>
