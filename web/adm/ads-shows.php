<?php
	...
	{   
		...
		$sql_ads = "SELECT * FROM ads WHERE position = 'shows'";
		...
		if($_GET) {
			if ($_GET['ac'] == "remover") {
				if ($_GET['di']) {
				    ...
				    header("Location:ads-shows.php?msg=remover");
				}
			}
		}
		...
		if($_POST) 
		{
			if($_FILES['image'])
			{
				...
				{   
					...
					{ 
						header("Location:ads-shows.php?msg=imagem");
					}  
					else
					{
						...
						if ($image_width == "600" && $image_height == "200") {
							...
							$caminho_imagem = "../images/top/shows/" . $nome_imagem;   
						
							move_uploaded_file($foto["tmp_name"], $caminho_imagem);   

							$sql_insert = "INSERT INTO ads (idstore, type, file, link, position) VALUEs (?, ?, ?, ?, 'shows')";
						    ...
						    header("Location:ads-shows.php?msg=sucesso");
						} else {
							header("Location:ads-shows.php?msg=tamanho");
						}
					}
				} else {
					header("Location:ads-shows.php?msg=name");					
				}
			} else {
				header("Location:ads-shows.php?msg=image");					
			}
		}
	}
	...
	<body>
		...
		<div class="container-fluid" style="margin: 0 auto; width: 1000px;">
			<div class="row">
				...
				<h1 class="page-header">Enviar anúncio em promoções, sorteios e eventos</h1>
				<div class="alert alert-info">Tamanho da imagem: 600 x 200 pixels</div>
				...
				<div id="menu">
					...
					<?php for($i = 0; $i < sizeof($ads); $i++) { ?>
						<div class="well well-sm" style="margin-bottom: 10px;">
							<a href="">
								<img class="media-object" src="../images/top/shows/<?php echo $ads[$i]['file']; ?>" style="width:600px;height:200px;margin-right:10px">
							</a>
							<div>
								<div class="media-body">
									...
									<a href="ads-shows.php?ac=remover&di=<?php echo $ads[$i]['id']; ?>" onclick="return confirm('Tem certeza?')" class="btn btn-danger">Remover</a>
								</div>
							</div>
							<div class="clear"></div>
						</div>
